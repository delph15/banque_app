...
const AjouterClient = ({ onNavigate, user, onLogout }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.numCompte.trim()) {
      newErrors.numCompte = 'Le numéro de compte est requis';
    } else if (!/^AEENI\d{3,}$/.test(formData.numCompte)) {
      newErrors.numCompte = 'Le format doit être AEENI suivi de chiffres';
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      setTimeout(() => {
        ...
        // Retourner à la liste des clients après succès
        onNavigate('clients');
      }, 2000);
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white shadow-sm border border-gray-200 rounded-lg p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Numéro de compte */}
            <div>
              <label htmlFor="numCompte" className="block text-sm font-medium text-gray-700 mb-2">
                Numéro de compte
              </label>
              <input
                type="text"
                ...
                className={`block w-full px-3 py-2 border rounded-lg shadow-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm ${
                  errors.numCompte ? 'border-red-300' : 'border-gray-300'
                }`}
              />
              {errors.numCompte && (
                <p className="mt-2 text-sm text-red-600">{errors.numCompte}</p>
              )}
            </div>
